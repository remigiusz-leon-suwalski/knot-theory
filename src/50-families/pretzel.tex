...
Wprowadził je do matematyki w~1973 roku \cite{montesinos1973}.

Kawauchi \cite[s. 29]{kawauchi1996} pisze, że Montesinos uogólnił precle, bo dwukrotne nakrycie nad $S^3$ rozgałęzione wzdłuż precla jest rozmaitością Seiferta.
...
Cały dwunasty rozdział podręcznika Burdego i~Zieschanga \cite{burde2014} jest poświęcony splotom Montesinosa.
...
Wtedy $K$, $ \operatorname{m} K$, $\operatorname{r} K$, $ \operatorname{mr} K$ są parami nierównoważne.
Węzeł $K \shrap  \operatorname{m} K$ jest dodatnio zwierciadlany, zaś $ \operatorname{r} K \shrap  \operatorname{m} K$ ujemnie zwierciadlany.
...
\begin{proposition}
    Niech $K$ będzie $(p_1, p_2, \ldots, p_n)$-preclem.
    Wtedy
    \begin{equation}
        \det K = \left(\prod_{k=1}^n p_k\right) \left( \sum_{k=1}^n \frac{1}{p_k}\right).
    \end{equation}
\end{proposition}

\begin{proof}
    Nie wiemy, kto pierwszy zauważył ten elementarny fakt, ale wiemy, gdzie pierwszy raz w~życiu o~tym przeczytaliśmy: w pracy Kolaya \cite[s. 14]{kolay2019}.
\end{proof}
...
\index{kolorowalność}%
    ...
    Jeśli przynajmniej jedna z~liczba $p, q, r$ nie jest wielokrotnością $n$, to kolorowanie z dokładnością do permutacji jest dokładnie jedno.
