...
Taki precel jest węzłem dokładnie wtedy, gdy $n$ oraz $p_i$ są nieparzyste lub dokładnie jedna z~liczb $p_i$ jest parzysta (Kawauchi \cite[s. 27]{kawauchi96}).
...
\index[persons]{Trotter, Hale}%
    Zgodnie z sugestią Foxa, Trotter \cite{trotter63} przetłumaczył problem na język teorii grup.
...
Z pracy Bankwitza, Schumanna \cite{schumann34} wynika, że jeśli któryś z parametrów ma wartość $\pm 1$, to węzeł też jest odwracalny.
...
\index[persons]{Lee, In}%
    Bae, Lee \cite[lemat 3.1]{bae20} pokazali, że macierz Seiferta $(p, q, r)$-precla to
    \begin{equation}
...
\index[persons]{Taalman, Laura}%
    ...
    Dowód drugiej zawiera praca Brownell, O'Neil, Taalman \cite[s. 11]{taalman05}.
\end{proof}

Podano tam (czyli w \cite{taalman05}) także ogólny wzór na liczbę $n$-kolorowań dowolnego węzła.
